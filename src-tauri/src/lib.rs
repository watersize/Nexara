...
/// Calls the Groq Chat Completions API directly using HTTP.
...
async fn call_groq(
    ...
    if state.groq_key.trim().is_empty() {
        return Err("GROQ_API_KEY не настроен. Добавьте ключ в настройки приложения.".to_string());
    }
    let payload = json!({
        "model": model,
        "messages": [
    ...
    let response = state
        ...
        .header("Content-Type", "application/json; charset=utf-8")
        .header("Authorization", format!("Bearer {}", state.groq_key))
        .json(&payload)
        .send()
        .await
        .map_err(|e| format!("Ошибка соединения с GROQ: {e}"))?;
    let status = response.status();
    ...
    v["choices"][0]["message"]["content"]
        ...
        .ok_or_else(|| "Пустой ответ от GROQ".to_string())
}
...
async fn save_schedule(payload: SaveSchedulePayload, state: State<'_, AppState>) -> Result<OperationResult, String> {
    ...
    let mut lessons = if !file_paths.is_empty() {
        ...
        #[cfg(mobile)]
        { return Err("Анализ файлов доступен только на десктопе.".to_string()); }
        ...
        #[cfg(mobile)]
        { return Err("Анализ текста доступен только на десктопе.".to_string()); }
    } else {
...
async fn ask_ai(question: String, state: State<'_, AppState>) -> Result<ChatResponse, String> {
    ...
    // On desktop: use Python agent with local RAG index for richer context.
    #[cfg(not(mobile))]
    {
        let storage_dir = state.rag_dir.join(user_key);
        let value = run_python_agent(
    ...
    // On mobile: call GROQ directly via HTTP — works everywhere without Python.
    #[cfg(mobile)]
    {
        let _ = user_key;
...
async fn generate_study_plan(
    ...
    #[cfg(mobile)]
    {
        let schedule_text = lessons
            .iter()
            .map(|l| format!("  • {}–{}: {} (каб. {})", l.start_time, l.end_time, l.subject, l.room))
            .collect::<Vec<_>>()
            .join("\n");
        let day = weekday_label(weekday);
        let user_prompt = format!(
            "Составь мотивирующий план на {day}. Расписание:\n{schedule_text}\n\nФормат Markdown: до школы, в школе, после уроков. Максимум 12 строк."
        );
        let system = "Ты дружелюбный школьный помощник. Пиши на русском, обращайся на 'ты'.";
        let plan = call_groq(&state, system, &user_prompt, "llama-3.1-8b-instant").await?;
