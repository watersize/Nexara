...
import { ChevronLeft, ChevronRight, Eye, EyeOff, FileOutput, Ghost, Grid3x3, ImagePlus, Layers3, Lock, NotebookPen, PenTool, Plus, Ruler, Save, Search, Sparkles, Table2, Unlock, Workflow } from 'lucide-react'
...
export default function NotebookPage() {
  ...
  const saveActive = async () => {
    if (!active) return
    await persist(active).catch(() => {})
  }

  return (
    <AppShell displayName={user?.displayName} email={user?.email}>
      <main className="flex-1 p-4 sm:p-6 xl:p-8">
        <div className="min-h-[calc(100vh-3rem)] rounded-[34px] border p-3 sm:p-4 xl:p-5" style={{ background: palette.shell, borderColor: palette.border, color: palette.text }}>
          <div className="mb-3 flex items-center justify-between rounded-[28px] border px-4 py-3" style={{ background: palette.panel, borderColor: palette.border }}>
            <div className="flex items-center gap-3">
              <div className="flex h-11 w-11 items-center justify-center rounded-[16px] border" style={{ background: palette.accentSoft, borderColor: palette.border }}><NotebookPen className="h-5 w-5" style={{ color: palette.accent }} /></div>
              <div>
                <div className="text-[11px] uppercase tracking-[0.26em]" style={{ color: palette.muted }}>veyo.ai</div>
                <div className="text-lg font-semibold">Hybrid workspace</div>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <div className="hidden min-w-[260px] items-center gap-3 rounded-[18px] border px-4 py-2.5 md:flex" style={{ background: palette.soft, borderColor: palette.border }}><Search className="h-4 w-4" style={{ color: palette.muted }} /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search notes, sessions and objects" className="border-none bg-transparent px-0 text-sm shadow-none focus-visible:ring-0" style={{ color: palette.text }} /></div>
              <Button onClick={() => void createNote()} className="h-11 rounded-[18px] px-4" style={{ background: palette.accent, color: dark ? '#08111f' : '#fff' }}><Plus className="h-4 w-4" />New note</Button>
            </div>
          </div>

          <div className="grid gap-3 xl:grid-cols-[44px_var(--library-width)_minmax(0,1fr)]" style={{ ['--library-width' as string]: libraryCollapsed ? '0px' : '328px' }}>
            <div className="flex min-h-[760px] items-start justify-center pt-2">
              <button type="button" onClick={() => setLibraryCollapsed((current) => !current)} className="flex h-11 w-11 items-center justify-center rounded-[16px] border transition" style={{ background: palette.panel, borderColor: palette.border, color: palette.text }}>
                {libraryCollapsed ? <ChevronRight className="h-4 w-4" /> : <ChevronLeft className="h-4 w-4" />}
              </button>
            </div>
            <div className={cn('overflow-hidden transition-[max-width,opacity,transform] duration-300 ease-out', libraryCollapsed ? 'pointer-events-none -translate-x-6 opacity-0' : 'translate-x-0 opacity-100')} style={{ maxWidth: libraryCollapsed ? 0 : 328 }}>
              <aside className="flex min-h-[760px] w-[328px] flex-col rounded-[30px] border p-4" style={{ background: palette.panel, borderColor: palette.border }}>
                <div className="mb-4 flex items-center justify-between"><div><div className="text-xs uppercase tracking-[0.24em]" style={{ color: palette.muted }}>Note Library</div><div className="mt-1 text-lg font-semibold">Knowledge Notes</div></div><div className="rounded-full px-3 py-1 text-xs font-semibold" style={{ background: palette.accentSoft, color: palette.accent }}>{filtered.length}</div></div>
                <div className="mb-4 flex items-center gap-3 rounded-[18px] border px-4 py-2.5 md:hidden" style={{ background: palette.soft, borderColor: palette.border }}><Search className="h-4 w-4" style={{ color: palette.muted }} /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search notes" className="border-none bg-transparent px-0 text-sm shadow-none focus-visible:ring-0" style={{ color: palette.text }} /></div>
                <div className="scrollbar-none grid flex-1 grid-cols-2 gap-3 overflow-y-auto pr-1">
                  {loading ? <div className="col-span-2 rounded-[24px] border px-4 py-8 text-sm" style={{ background: palette.soft, borderColor: palette.border, color: palette.muted }}>Loading workspace notes...</div> : filtered.map((note, index) => (
                    <button key={note.id} type="button" onClick={() => startTransition(() => { const firstId = note.doc.objects[0]?.id || ''; setActiveId(note.id); setStage('preview'); setSelectedId(firstId); setSelectedIds(firstId ? [firstId] : []) })} className={cn('rounded-[26px] border p-4 text-left transition', note.id === active?.id ? 'translate-y-0 shadow-[0_18px_40px_rgba(7,11,20,0.32)]' : 'hover:-translate-y-0.5')} style={{ minHeight: 188, background: note.id === active?.id ? palette.strong : palette.soft, borderColor: note.id === active?.id ? palette.accent : palette.border }}>
                      <div className="text-[12px]" style={{ color: palette.muted }}>Note Title</div>
                      <div className="mt-1 line-clamp-2 text-[30px] font-semibold leading-none tracking-[-0.04em]">{note.title}</div>
                      <div className="mt-3 text-xs uppercase tracking-[0.16em]" style={{ color: palette.muted }}>{format(new Date(note.updatedAt), index < 4 ? 'd MMM. yyyy' : 'd MMM yyyy, HH:mm', { locale: ru })}</div>
                      <div className="mt-4 line-clamp-4 border-t pt-4 text-sm leading-6" style={{ color: palette.muted, borderColor: palette.border }}>{snippet(note.doc)}</div>
                    </button>
                  ))}
                </div>
              </aside>
            ...
            {!active ? <div className="flex min-h-[760px] items-center justify-center rounded-[30px] border p-8 text-center text-sm" style={{ background: palette.panel, borderColor: palette.border, color: palette.muted }}>Select a workspace note from the library.</div> : stage === 'preview' ? (
              ...
              <div className="grid min-h-[760px] gap-3 rounded-[30px] border p-3 xl:grid-cols-[minmax(0,1fr)_280px]" style={{ background: palette.panel, borderColor: palette.border }}>
                <div className="flex min-h-[720px] flex-col gap-3">
                  <div className="relative rounded-[28px] border px-5 py-4" style={{ background: palette.strong, borderColor: palette.border }}>
                    <div className="flex items-center justify-between gap-3">
                      <button type="button" onClick={() => setStage('preview')} className="inline-flex h-11 w-11 items-center justify-center rounded-[16px] border" style={{ background: palette.soft, borderColor: palette.border, color: palette.text }}>
                        <ChevronLeft className="h-4 w-4" />
                      </button>
                      <button type="button" onClick={() => setMetaPanelOpen((current) => !current)} className="min-w-0 flex-1 text-center">
                        <div className="truncate text-[34px] font-semibold leading-none tracking-[-0.04em]">{active.title}</div>
                        <div className="mt-1 truncate text-lg" style={{ color: palette.muted }}>{active.topic}</div>
                      </button>
                      <div className="flex items-center gap-2">
                        <button type="button" onClick={() => exportNotePdf(active, dark)} className="inline-flex items-center gap-2 rounded-[16px] border px-4 py-2.5 text-sm font-semibold" style={{ background: palette.soft, color: palette.text, borderColor: palette.border }}><FileOutput className="h-4 w-4" />PDF exports</button>
                        <button type="button" onClick={() => void saveActive()} className="inline-flex items-center gap-2 rounded-[16px] border px-4 py-2.5 text-sm font-semibold" style={{ background: palette.accent, color: dark ? '#08111f' : '#fff', borderColor: palette.accent }}><Save className="h-4 w-4" />Save</button>
                      </div>
                    </div>
                    {metaPanelOpen ? <div className="absolute left-1/2 top-[calc(100%+12px)] z-20 w-[min(560px,92vw)] -translate-x-1/2 rounded-[24px] border p-4 shadow-[0_24px_80px_rgba(3,6,14,0.34)]" style={{ background: palette.panel, borderColor: palette.border }}><div className="grid gap-3 sm:grid-cols-2"><div><div className="mb-2 text-xs uppercase tracking-[0.18em]" style={{ color: palette.muted }}>Notebook Title</div><Input value={active.title} onChange={(event) => mutate((note) => ({ ...note, title: event.target.value }))} className="h-12 rounded-[16px] border" style={{ background: palette.strong, borderColor: palette.border, color: palette.text }} /></div><div><div className="mb-2 text-xs uppercase tracking-[0.18em]" style={{ color: palette.muted }}>Topic / Project</div><Input value={active.topic} onChange={(event) => mutate((note) => ({ ...note, topic: event.target.value }))} className="h-12 rounded-[16px] border" style={{ background: palette.strong, borderColor: palette.border, color: palette.text }} /></div></div></div> : null}
                  </div>
                  <div className="grid min-h-[640px] gap-3 xl:grid-cols-[250px_minmax(0,1fr)]">
                <div className="flex min-h-[640px] flex-col rounded-[28px] border p-3" style={{ background: palette.soft, borderColor: palette.border }}>
                ...
                <div className="flex min-h-[720px] flex-col gap-4">
                  <div className="rounded-[28px] border p-3" style={{ background: palette.soft, borderColor: palette.border }}>
                    <div className="flex flex-wrap gap-2">{[{ label: active.doc.prefs.dimMode ? 'Dimension Mode' : 'Sketch Mode', icon: Ruler, onClick: () => patchPrefs((prefs) => ({ ...prefs, dimMode: !prefs.dimMode })) }, { label: active.doc.prefs.dims ? 'Hide Dimensions' : 'Show Dimensions', icon: active.doc.prefs.dims ? Eye : EyeOff, onClick: () => patchPrefs((prefs) => ({ ...prefs, dims: !prefs.dims })) }, { label: active.doc.prefs.grid, icon: Grid3x3, onClick: () => patchPrefs((prefs) => ({ ...prefs, grid: prefs.grid === 'off' ? 'orthographic' : prefs.grid === 'orthographic' ? 'isometric' : 'off' })) }, { label: active.doc.prefs.ghost ? 'Ghost On' : 'Ghost Off', icon: Ghost, onClick: () => patchPrefs((prefs) => ({ ...prefs, ghost: !prefs.ghost })) }, { label: 'Duplicate', icon: Plus, onClick: () => selected && addObject({ ...selected, id: makeId(selected.type), x: selected.x + 24, y: selected.y + 24, z: active.doc.objects.length + 2 }) }, { label: 'Delete', icon: Lock, onClick: () => selectedIds.length && mutate((note) => ({ ...note, doc: { ...note.doc, objects: note.doc.objects.filter((object) => !selectedIds.includes(object.id)) } })) }].map((item) => { const Icon = item.icon; return <button key={item.label} type="button" onClick={item.onClick} className="inline-flex items-center gap-2 rounded-[16px] border px-3 py-2 text-sm font-semibold" style={{ background: palette.strong, color: palette.text, borderColor: palette.border }}><Icon className="h-4 w-4" />{item.label}</button> })}</div>
                  </div>
                  <div className="relative flex-1 overflow-hidden rounded-[30px] border" style={{ background: palette.page, borderColor: palette.border }}>
                    <div className="border-b px-5 py-4" style={{ borderColor: palette.border, background: palette.strong }}>
                      <div className="flex flex-wrap items-center justify-center gap-2">{toolbarItems.map((item) => { const Icon = item.icon; return <button key={item.key} type="button" onClick={() => setTool(item.key)} className="inline-flex items-center gap-2 rounded-[16px] border px-4 py-2.5 text-sm font-medium transition" style={{ background: tool === item.key ? palette.accentSoft : palette.soft, color: tool === item.key ? palette.accent : palette.text, borderColor: tool === item.key ? palette.accent : palette.border }}><Icon className="h-4 w-4" />{item.label}</button> })}</div>
                      <p className="mt-4 max-w-4xl text-base leading-7" style={{ color: palette.muted }}>Detailed engineering schemas and structured drawings, clean typography, trace-ready assets and markdown-like documentation blocks arranged inside one hybrid workspace.</p>
                    </div>
                    <div className="grid h-[calc(100%-126px)] grid-rows-[minmax(0,1fr)_188px]">
